import { createIcon } from '../utils';

// Velodrome Finance icon mark — curved swoosh + circular dot
// Sourced from @web3icons/react (MIT), viewBox 0 0 24 24
const VELODROME_PATH =
  'M12.667 18.353c-.828-.02-1.557-.295-2.149-.886-2.188-2.203-4.396-4.387-6.603-6.59C2.279 9.242 2.929 6.488 5.136 5.8c1.164-.354 2.228-.099 3.096.767 2.188 2.164 4.337 4.328 6.505 6.493.592.59.927 1.298.966 2.144.06 1.397-.946 2.715-2.326 3.03-.217.058-.474.079-.71.118m5.44-12.706A2.88 2.88 0 0 1 21 8.539a2.915 2.915 0 0 1-2.893 2.931c-1.597.019-2.912-1.315-2.93-2.911a2.93 2.93 0 0 1 2.93-2.912';

const velodromeContent = () => <path d={VELODROME_PATH} />;

/** Velodrome DEX icon (colored). */
export const Velodrome = /* @__PURE__ */ createIcon(
  'Velodrome',
  '0 0 24 24',
  velodromeContent,
  '#DB2264',
);

/** Velodrome DEX icon (monochrome). */
export const VelodromeMono = /* @__PURE__ */ createIcon(
  'VelodromeMono',
  '0 0 24 24',
  velodromeContent,
  'currentColor',
);
